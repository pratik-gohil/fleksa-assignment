...
export interface IMenuSliceState {
  ...
  viewableCategoreis: Array<ICategory>;
...
export const MenuSlice = createSlice({
  ...
  reducers: {
    ...
    updateMenuViewableCategories(state, action) {
      state.viewableCategoreis = action.payload;
    },
  },
  extraReducers: {
...
export const selectCategoriesSearch = (state: RootState, searchQuery: string) => {
  if (searchQuery.length === 0) {
    ...
    return state.menu.viewableCategoreis.map((category) => {
...
export const selectMenuCategories = (state: RootState) => state.menu.categories;
export const selectViewableMenuCategories = (state: RootState) => state.menu.viewableCategoreis;
